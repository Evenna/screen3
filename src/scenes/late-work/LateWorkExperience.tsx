...
function DataCard({ time }: { time: number }) {
  const opacity = visibilityBetween(time, 6.2, 23.2, 0.65)
  if (opacity <= 0.01) return null

  return (
    <Html position={[0.0, 2.0, 0.1]} center distanceFactor={8.4} transform sprite occlude={false} zIndexRange={[42, 0]}>
      <div className="late-work-data-card" style={{ opacity, transform: `translateY(${6 - opacity * 6}px) scale(${0.7 + opacity * 0.08})` }}>
        <div className="late-work-card-topbar">
          <span>Evans Health Insight / 01:47</span>
          <i>近 90 天数据</i>
        </div>
        <header>
          <small>不是阻止你，是给你证据</small>
          <strong>你自己的产出数据</strong>
        </header>
        <div className="late-work-stats">
          <span><b>78%</b>凌晨后产出，第二天返工</span>
          <span><b>84%</b>凌晨改方案，第二天大改</span>
          <span><b>67%</b>凌晨发邮件，第二天后悔</span>
        </div>
        <section>
          <b>最佳窗口</b>
          <p>09:00-11:00 是你的最佳产出时段；当前方案已完成 92%，剩余 8% 明早 30 分钟可完成。</p>
        </section>
      </div>
    </Html>
  )
...
function ActionPanel({ time }: { time: number }) {
  const opacity = visibilityBetween(time, 14.4, 34.5, 0.62)
  if (opacity <= 0.01) return null

  return (
    <Html position={[1.62, 1.58, 0.48]} center distanceFactor={8.7} transform sprite occlude={false} zIndexRange={[40, 0]}>
      <div className="late-work-action-card" style={{ opacity, transform: `translateY(${5 - opacity * 5}px) scale(${0.72 + opacity * 0.07})` }}>
        <strong>已为你安排好</strong>
        <span>闹钟 06:30 已设置 · 柔和铃声</span>
        <span>日历 07:00-07:30 · 完成最后 8%</span>
        <span>邮件草稿已起草 · 明日 09:00 定时</span>
        <span>Slack 睡眠模式 · 持续至 08:00</span>
      </div>
    </Html>
  )
}
...
function DialogueLayer({ time }: { time: number }) {
  const entries = [
    ...
    },
    {
      ...
      kind: 'voice',
      title: 'Evans',
      body: '李明，我得告诉你一件事：你最近 90 天，凌晨 0 点后改的方案，第二天大改概率 84%。',
    ...
    },
    {
      ...
      kind: 'voice',
      title: 'Evans',
      body: '剩下的 8% 是排版和数据图，明早 30 分钟就能搞定。现在睡，6:30 我叫你起来。',
      position: [-0.44, 1.72, 0.9] as Vec3,
    },
    {
      start: 15.2,
      ...
      kind: 'voice',
      title: 'Evans',
      body: '或者你坚持改完，我也不拦你。但我想让你知道你自己的数据。',
      position: [-0.44, 1.72, 0.9] as Vec3,
    },
  ]
      ...
      {entries.map((entry) => {
        ...
        return (
          <Html key={entry.start} position={entry.position} center distanceFactor={7.8} transform sprite occlude={false} zIndexRange={[38, 0]}>
            <div className={`object-label object-label--${entry.kind} late-work-dialog`} style={{ opacity, transform: `translateY(${8 - opacity * 8}px) scale(${0.94 + opacity * 0.06})` }}>
...
function DeviceStatus({ time }: { time: number }) {
  const lampOpacity = visibilityBetween(time, 16.0, 27.5, 0.55)
  if (lampOpacity <= 0.01) return null

  return (
    <Html position={[-1.13, 1.48, -0.18]} center distanceFactor={7.8} transform sprite occlude={false} zIndexRange={[36, 0]}>
      <div className="object-label object-label--action late-work-device-note" style={{ opacity: lampOpacity }}>
        <strong>智能台灯</strong>
        冷白 5500K → 暖橙 2700K<br />
        亮度 80% → 30%
      </div>
    </Html>
  )
...
function DispatchMotion({ time }: { time: number }) {
  ...
  const dataCard = useMemo(() => new THREE.Vector3(0, 1.88, 0.14), [])
  const scheduleCard = useMemo(() => new THREE.Vector3(1.08, 1.9, 0.18), [])
  const mailCard = useMemo(() => new THREE.Vector3(1.48, 1.52, 0.4), [])
  const lamp = useMemo(() => new THREE.Vector3(-0.84, 1.38, -0.23), [])
  const slack = useMemo(() => new THREE.Vector3(1.58, 1.28, 0.62), [])

  return (
    <group>
      <AnimatedRoute points={[brooch, new THREE.Vector3(-0.18, 1.65, 0.32), dataCard]} time={time} start={5.4} end={6.8} holdUntil={23.5} color={MAIN_BLUE} width={0.006} opacity={0.72} />
      <AnimatedRoute points={[brooch, new THREE.Vector3(0.66, 1.72, 0.18), scheduleCard]} time={time} start={10.2} end={11.5} holdUntil={18.6} color={MAIN_BLUE} width={0.0054} opacity={0.62} />
      <AnimatedRoute points={[brooch, new THREE.Vector3(0.92, 1.48, 0.38), mailCard]} time={time} start={11.0} end={12.5} holdUntil={19.4} color={MAIN_BLUE} width={0.0054} opacity={0.62} />
      <AnimatedRoute points={[brooch, new THREE.Vector3(-0.46, 1.42, 0.08), lamp]} time={time} start={15.4} end={16.6} holdUntil={27.5} color={ACTIVE_BLUE} width={0.0048} opacity={0.48} />
      <AnimatedRoute points={[brooch, new THREE.Vector3(0.84, 1.36, 0.62), slack]} time={time} start={16.2} end={17.6} holdUntil={27.5} color={ACTIVE_BLUE} width={0.0048} opacity={0.48} />
      <DataCard time={time} />
...
function LateWorkSet({ time }: { time: number }) {
  return (
    <group>
      <StudyRoomShell />
...
export function LateWorkExperience() {
  ...
  return (
    <section className="interior-shell" aria-label="深夜书房李明加班中断三维静态场景">
